...
def extract_pose(calib_info):
    pose_matrix = [calib_info.stDepth2RgbExtrinsic.fData[i] for i in range(12)]
    pose_matrix = np.array(pose_matrix).reshape((3, 4)).astype(np.float64)
    return pose_matrix
...
if __name__ == "__main__":
    ...
    param =  MV3D_RGBD_CAMERA_PARAM()
    camera.MV3D_RGBD_GetCameraParam(pointer(param))
    pose = extract_pose(param)
    # np.save(os.path.join(save_dir, 'pose', f'{stData.nFrameNum}.npy'), pose)
    ...
    while True:
        ...
        if ret==0:
            for i in range(0, stFrameData.nImageCount):
                ...
                # point = np.frombuffer(pp1,dtype=np.float32).reshape(-1, 3)

                # # 点云数据处理                                                                                                                                                                                                                                 
                # mask = np.any(point != 0, axis=1)
                ...
                # scaled_point = np.tile(scaled_point, (1, 2)) # dummy xyzrgb
                # np.save(os.path.join(save_dir, 'point', f"{stData.nFrameNum}.npy"), scaled_point)
                # print(f"{stData.nFrameNum}.npy shape:", scaled_point.shape)
                ...
                if i == 0:
                    ...
                    cv2.imwrite(os.path.join(save_dir, 'image', f'{stData.nFrameNum}.jpg'), image)
